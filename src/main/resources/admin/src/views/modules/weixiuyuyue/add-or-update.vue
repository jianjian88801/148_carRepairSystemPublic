    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="维修项目" prop="weixiuxiangmu">
          <el-select  @change="weixiuxiangmuChange" v-model="ruleForm.weixiuxiangmu" placeholder="请选择维修项目">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.weixiuxiangmu" label="维修项目" prop="weixiuxiangmu">
...
export default {
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.shouji!=''&&json.shouji){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/weixiuxiangmu/weixiuxiangmu`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
    ...
    weixiuxiangmuChange () {
      this.$http({
        url: `follow/weixiuxiangmu/weixiuxiangmu?columnValue=`+ this.ruleForm.weixiuxiangmu,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.jiage){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `weixiuyuyue/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.weixiuyuyueCrossAddOrUpdateFlag = false;
                  this.parent.search();
